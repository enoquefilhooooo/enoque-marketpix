...
export default function Checkout(){
  ...
  useEffect(()=>{try{setItems(JSON.parse(localStorage.getItem('marketpix-cart')||'[]'));setPixKey(localStorage.getItem('marketpix-pix-key')||'')}catch{}}
  const total=items.reduce((s,i)=>s+i.price*i.qty,0)
  const pixText=pixKey||'Configure sua chave PIX no painel administrativo'
  function copy(){if(!pixKey)return;navigator.clipboard?.writeText(pixKey);setCopied(true);setTimeout(()=>setCopied(false),1800)}
